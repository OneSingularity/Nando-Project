#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: StaticMeshDescription

#include "Basic.hpp"

#include "MeshDescription_structs.hpp"
#include "CoreUObject_structs.hpp"


namespace SDK::Params
{

// Function StaticMeshDescription.StaticMeshDescription.CreateCube
// 0x0050 (0x0050 - 0x0000)
struct StaticMeshDescription_CreateCube final
{
public:
	struct FVector                                Center;                                            // 0x0000(0x0018)
	struct FVector                                HalfExtents;                                       // 0x0018(0x0018)
	struct FPolygonGroupID                        PolygonGroup;                                      // 0x0030(0x0004)
	struct FPolygonID                             PolygonID_PlusX;                                   // 0x0034(0x0004)
	struct FPolygonID                             PolygonID_MinusX;                                  // 0x0038(0x0004)
	struct FPolygonID                             PolygonID_PlusY;                                   // 0x003C(0x0004)
	struct FPolygonID                             PolygonID_MinusY;                                  // 0x0040(0x0004)
	struct FPolygonID                             PolygonID_PlusZ;                                   // 0x0044(0x0004)
	struct FPolygonID                             PolygonID_MinusZ;                                  // 0x0048(0x0004)
	uint8                                         Pad_4C[0x4];                                       // 0x004C(0x0004)
};

// Function StaticMeshDescription.StaticMeshDescription.SetPolygonGroupMaterialSlotName
// 0x000C (0x000C - 0x0000)
struct StaticMeshDescription_SetPolygonGroupMaterialSlotName final
{
public:
	struct FPolygonGroupID                        PolygonGroupID;                                    // 0x0000(0x0004)
	class FName                                   SlotName;                                          // 0x0004(0x0008)
};

// Function StaticMeshDescription.StaticMeshDescription.SetVertexInstanceUV
// 0x0020 (0x0020 - 0x0000)
struct StaticMeshDescription_SetVertexInstanceUV final
{
public:
	struct FVertexInstanceID                      VertexInstanceID;                                  // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FVector2D                              UV;                                                // 0x0008(0x0010)
	int32                                         UVIndex;                                           // 0x0018(0x0004)
	uint8                                         Pad_1C[0x4];                                       // 0x001C(0x0004)
};

// Function StaticMeshDescription.StaticMeshDescription.GetVertexInstanceUV
// 0x0018 (0x0018 - 0x0000)
struct StaticMeshDescription_GetVertexInstanceUV final
{
public:
	struct FVertexInstanceID                      VertexInstanceID;                                  // 0x0000(0x0004)
	int32                                         UVIndex;                                           // 0x0004(0x0004)
	struct FVector2D                              ReturnValue;                                       // 0x0008(0x0010)
};

}

