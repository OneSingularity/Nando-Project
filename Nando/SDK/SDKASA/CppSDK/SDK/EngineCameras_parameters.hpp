#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: EngineCameras

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "EngineCameras_structs.hpp"
#include "CoreUObject_structs.hpp"


namespace SDK::Params
{

// Function EngineCameras.LegacyCameraShake.StartLegacyCameraShake
// 0x0038 (0x0038 - 0x0000)
struct LegacyCameraShake_StartLegacyCameraShake final
{
public:
	class APlayerCameraManager*                   PlayerCameraManager;                               // 0x0000(0x0008)
	TSubclassOf<class ULegacyCameraShake>         ShakeClass;                                        // 0x0008(0x0008)
	float                                         Scale;                                             // 0x0010(0x0004)
	ECameraShakePlaySpace                         PlaySpace;                                         // 0x0014(0x0001)
	uint8                                         Pad_15[0x3];                                       // 0x0015(0x0003)
	struct FRotator                               UserPlaySpaceRot;                                  // 0x0018(0x0018)
	class ULegacyCameraShake*                     ReturnValue;                                       // 0x0030(0x0008)
};

// Function EngineCameras.LegacyCameraShake.StartLegacyCameraShakeFromSource
// 0x0040 (0x0040 - 0x0000)
struct LegacyCameraShake_StartLegacyCameraShakeFromSource final
{
public:
	class APlayerCameraManager*                   PlayerCameraManager;                               // 0x0000(0x0008)
	TSubclassOf<class ULegacyCameraShake>         ShakeClass;                                        // 0x0008(0x0008)
	class UCameraShakeSourceComponent*            SourceComponent;                                   // 0x0010(0x0008)
	float                                         Scale;                                             // 0x0018(0x0004)
	ECameraShakePlaySpace                         PlaySpace;                                         // 0x001C(0x0001)
	uint8                                         Pad_1D[0x3];                                       // 0x001D(0x0003)
	struct FRotator                               UserPlaySpaceRot;                                  // 0x0020(0x0018)
	class ULegacyCameraShake*                     ReturnValue;                                       // 0x0038(0x0008)
};

// Function EngineCameras.LegacyCameraShake.BlueprintUpdateCameraShake
// 0x12D0 (0x12D0 - 0x0000)
struct LegacyCameraShake_BlueprintUpdateCameraShake final
{
public:
	float                                         DeltaTime;                                         // 0x0000(0x0004)
	float                                         Alpha;                                             // 0x0004(0x0004)
	uint8                                         Pad_8[0x8];                                        // 0x0008(0x0008)
	struct FMinimalViewInfo                       POV;                                               // 0x0010(0x0960)
	struct FMinimalViewInfo                       ModifiedPOV;                                       // 0x0970(0x0960)
};

// Function EngineCameras.LegacyCameraShake.ReceivePlayShake
// 0x0004 (0x0004 - 0x0000)
struct LegacyCameraShake_ReceivePlayShake final
{
public:
	float                                         Scale;                                             // 0x0000(0x0004)
};

// Function EngineCameras.LegacyCameraShake.ReceiveStopShake
// 0x0001 (0x0001 - 0x0000)
struct LegacyCameraShake_ReceiveStopShake final
{
public:
	bool                                          bImmediately;                                      // 0x0000(0x0001)
};

// Function EngineCameras.LegacyCameraShake.ReceiveIsFinished
// 0x0001 (0x0001 - 0x0000)
struct LegacyCameraShake_ReceiveIsFinished final
{
public:
	bool                                          ReturnValue;                                       // 0x0000(0x0001)
};

// Function EngineCameras.LegacyCameraShakeFunctionLibrary.Conv_LegacyCameraShake
// 0x0010 (0x0010 - 0x0000)
struct LegacyCameraShakeFunctionLibrary_Conv_LegacyCameraShake final
{
public:
	class UCameraShakeBase*                       CameraShake;                                       // 0x0000(0x0008)
	class ULegacyCameraShake*                     ReturnValue;                                       // 0x0008(0x0008)
};

// Function EngineCameras.CameraAnimationCameraModifier.GetCameraAnimationCameraModifier
// 0x0018 (0x0018 - 0x0000)
struct CameraAnimationCameraModifier_GetCameraAnimationCameraModifier final
{
public:
	const class UObject*                          WorldContextObject;                                // 0x0000(0x0008)
	int32                                         PlayerIndex;                                       // 0x0008(0x0004)
	uint8                                         Pad_C[0x4];                                        // 0x000C(0x0004)
	class UCameraAnimationCameraModifier*         ReturnValue;                                       // 0x0010(0x0008)
};

// Function EngineCameras.CameraAnimationCameraModifier.GetCameraAnimationCameraModifierFromID
// 0x0018 (0x0018 - 0x0000)
struct CameraAnimationCameraModifier_GetCameraAnimationCameraModifierFromID final
{
public:
	const class UObject*                          WorldContextObject;                                // 0x0000(0x0008)
	int32                                         ControllerId;                                      // 0x0008(0x0004)
	uint8                                         Pad_C[0x4];                                        // 0x000C(0x0004)
	class UCameraAnimationCameraModifier*         ReturnValue;                                       // 0x0010(0x0008)
};

// Function EngineCameras.CameraAnimationCameraModifier.GetCameraAnimationCameraModifierFromPlayerController
// 0x0010 (0x0010 - 0x0000)
struct CameraAnimationCameraModifier_GetCameraAnimationCameraModifierFromPlayerController final
{
public:
	const class APlayerController*                PlayerController;                                  // 0x0000(0x0008)
	class UCameraAnimationCameraModifier*         ReturnValue;                                       // 0x0008(0x0008)
};

// Function EngineCameras.CameraAnimationCameraModifier.PlayCameraAnimation
// 0x0058 (0x0058 - 0x0000)
struct CameraAnimationCameraModifier_PlayCameraAnimation final
{
public:
	class UCameraAnimationSequence*               Sequence;                                          // 0x0000(0x0008)
	struct FCameraAnimationParams                 Params_0;                                          // 0x0008(0x0048)
	struct FCameraAnimationHandle                 ReturnValue;                                       // 0x0050(0x0004)
	uint8                                         Pad_54[0x4];                                       // 0x0054(0x0004)
};

// Function EngineCameras.CameraAnimationCameraModifier.StopAllCameraAnimations
// 0x0001 (0x0001 - 0x0000)
struct CameraAnimationCameraModifier_StopAllCameraAnimations final
{
public:
	bool                                          bImmediate;                                        // 0x0000(0x0001)
};

// Function EngineCameras.CameraAnimationCameraModifier.StopAllCameraAnimationsOf
// 0x0010 (0x0010 - 0x0000)
struct CameraAnimationCameraModifier_StopAllCameraAnimationsOf final
{
public:
	class UCameraAnimationSequence*               Sequence;                                          // 0x0000(0x0008)
	bool                                          bImmediate;                                        // 0x0008(0x0001)
	uint8                                         Pad_9[0x7];                                        // 0x0009(0x0007)
};

// Function EngineCameras.CameraAnimationCameraModifier.StopCameraAnimation
// 0x0006 (0x0006 - 0x0000)
struct CameraAnimationCameraModifier_StopCameraAnimation final
{
public:
	struct FCameraAnimationHandle                 Handle;                                            // 0x0000(0x0004)
	bool                                          bImmediate;                                        // 0x0004(0x0001)
	uint8                                         Pad_5[0x1];                                        // 0x0005(0x0001)
};

// Function EngineCameras.CameraAnimationCameraModifier.IsCameraAnimationActive
// 0x0006 (0x0006 - 0x0000)
struct CameraAnimationCameraModifier_IsCameraAnimationActive final
{
public:
	struct FCameraAnimationHandle                 Handle;                                            // 0x0000(0x0004)
	bool                                          ReturnValue;                                       // 0x0004(0x0001)
	uint8                                         Pad_5[0x1];                                        // 0x0005(0x0001)
};

// Function EngineCameras.EngineCameraAnimationFunctionLibrary.Conv_CameraAnimationCameraModifier
// 0x0010 (0x0010 - 0x0000)
struct EngineCameraAnimationFunctionLibrary_Conv_CameraAnimationCameraModifier final
{
public:
	class APlayerCameraManager*                   PlayerCameraManager;                               // 0x0000(0x0008)
	class UCameraAnimationCameraModifier*         ReturnValue;                                       // 0x0008(0x0008)
};

// Function EngineCameras.EngineCameraAnimationFunctionLibrary.Conv_CameraAnimationPlaySpace
// 0x0002 (0x0002 - 0x0000)
struct EngineCameraAnimationFunctionLibrary_Conv_CameraAnimationPlaySpace final
{
public:
	ECameraShakePlaySpace                         CameraShakePlaySpace;                              // 0x0000(0x0001)
	ECameraAnimationPlaySpace                     ReturnValue;                                       // 0x0001(0x0001)
};

// Function EngineCameras.EngineCameraAnimationFunctionLibrary.Conv_CameraShakePlaySpace
// 0x0002 (0x0002 - 0x0000)
struct EngineCameraAnimationFunctionLibrary_Conv_CameraShakePlaySpace final
{
public:
	ECameraAnimationPlaySpace                     CameraAnimationPlaySpace;                          // 0x0000(0x0001)
	ECameraShakePlaySpace                         ReturnValue;                                       // 0x0001(0x0001)
};

// Function EngineCameras.EngineCamerasSubsystem.PlayCameraAnimation
// 0x0060 (0x0060 - 0x0000)
struct EngineCamerasSubsystem_PlayCameraAnimation final
{
public:
	class APlayerController*                      PlayerController;                                  // 0x0000(0x0008)
	class UCameraAnimationSequence*               Sequence;                                          // 0x0008(0x0008)
	struct FCameraAnimationParams                 Params_0;                                          // 0x0010(0x0048)
	struct FCameraAnimationHandle                 ReturnValue;                                       // 0x0058(0x0004)
	uint8                                         Pad_5C[0x4];                                       // 0x005C(0x0004)
};

// Function EngineCameras.EngineCamerasSubsystem.StopAllCameraAnimations
// 0x0010 (0x0010 - 0x0000)
struct EngineCamerasSubsystem_StopAllCameraAnimations final
{
public:
	class APlayerController*                      PlayerController;                                  // 0x0000(0x0008)
	bool                                          bImmediate;                                        // 0x0008(0x0001)
	uint8                                         Pad_9[0x7];                                        // 0x0009(0x0007)
};

// Function EngineCameras.EngineCamerasSubsystem.StopAllCameraAnimationsOf
// 0x0018 (0x0018 - 0x0000)
struct EngineCamerasSubsystem_StopAllCameraAnimationsOf final
{
public:
	class APlayerController*                      PlayerController;                                  // 0x0000(0x0008)
	class UCameraAnimationSequence*               Sequence;                                          // 0x0008(0x0008)
	bool                                          bImmediate;                                        // 0x0010(0x0001)
	uint8                                         Pad_11[0x7];                                       // 0x0011(0x0007)
};

// Function EngineCameras.EngineCamerasSubsystem.StopCameraAnimation
// 0x0010 (0x0010 - 0x0000)
struct EngineCamerasSubsystem_StopCameraAnimation final
{
public:
	class APlayerController*                      PlayerController;                                  // 0x0000(0x0008)
	struct FCameraAnimationHandle                 Handle;                                            // 0x0008(0x0004)
	bool                                          bImmediate;                                        // 0x000C(0x0001)
	uint8                                         Pad_D[0x3];                                        // 0x000D(0x0003)
};

// Function EngineCameras.EngineCamerasSubsystem.IsCameraAnimationActive
// 0x0010 (0x0010 - 0x0000)
struct EngineCamerasSubsystem_IsCameraAnimationActive final
{
public:
	class APlayerController*                      PlayerController;                                  // 0x0000(0x0008)
	struct FCameraAnimationHandle                 Handle;                                            // 0x0008(0x0004)
	bool                                          ReturnValue;                                       // 0x000C(0x0001)
	uint8                                         Pad_D[0x3];                                        // 0x000D(0x0003)
};

}

