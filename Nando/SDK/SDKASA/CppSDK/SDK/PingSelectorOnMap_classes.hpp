#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PingSelectorOnMap

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "ShooterGame_structs.hpp"
#include "ShooterGame_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass PingSelectorOnMap.PingSelectorOnMap_C
// 0x0068 (0x03E0 - 0x0378)
class UPingSelectorOnMap_C final : public UPrimalUserWidget
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0378(0x0008)
	class UButton*                                Attack;                                            // 0x0380(0x0008)
	class UButton*                                Danger;                                            // 0x0388(0x0008)
	class UButton*                                Follow;                                            // 0x0390(0x0008)
	class UButton*                                GoHere;                                            // 0x0398(0x0008)
	class UImage*                                 Image;                                             // 0x03A0(0x0008)
	class UImage*                                 Image_1;                                           // 0x03A8(0x0008)
	class UImage*                                 Image_2;                                           // 0x03B0(0x0008)
	class UImage*                                 Image_3;                                           // 0x03B8(0x0008)
	class UImage*                                 Image_4;                                           // 0x03C0(0x0008)
	class UButton*                                RallyPointButton;                                  // 0x03C8(0x0008)
	TMulticastInlineDelegate<void(ETeamPingType PingType)> CallPing;                                 // 0x03D0(0x0010)

public:
	void BndEvt__PingSelectorOnMap_Attack_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature();
	void BndEvt__PingSelectorOnMap_Danger_K2Node_ComponentBoundEvent_2_OnButtonClickedEvent__DelegateSignature();
	void BndEvt__PingSelectorOnMap_Follow_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature();
	void BndEvt__PingSelectorOnMap_GoHere_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature();
	void BndEvt__PingSelectorOnMap_RallyPointButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature();
	void ExecuteUbergraph_PingSelectorOnMap(int32 EntryPoint);

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("PingSelectorOnMap_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"PingSelectorOnMap_C")
	}
	static class UPingSelectorOnMap_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPingSelectorOnMap_C>();
	}
};

}

