#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: MassSmartObjects

#include "Basic.hpp"

#include "GameplayTags_structs.hpp"
#include "SmartObjectsModule_structs.hpp"
#include "MassEntity_structs.hpp"
#include "CoreUObject_structs.hpp"
#include "ZoneGraph_structs.hpp"


namespace SDK
{

// Enum MassSmartObjects.EMassSmartObjectInteractionStatus
// NumValues: 0x0006
enum class EMassSmartObjectInteractionStatus : uint8
{
	Unset                                    = 0,
	InProgress                               = 1,
	BehaviorCompleted                        = 2,
	TaskCompleted                            = 3,
	Aborted                                  = 4,
	EMassSmartObjectInteractionStatus_MAX    = 5,
};

// ScriptStruct MassSmartObjects.MassSmartObjectUserFragment
// 0x0050 (0x0050 - 0x0000)
struct FMassSmartObjectUserFragment final : public FMassFragment
{
public:
	struct FGameplayTagContainer                  UserTags;                                          // 0x0000(0x0020)
	struct FSmartObjectClaimHandle                InteractionHandle;                                 // 0x0020(0x0020)
	EMassSmartObjectInteractionStatus             InteractionStatus;                                 // 0x0040(0x0001)
	uint8                                         Pad_41[0x7];                                       // 0x0041(0x0007)
	double                                        InteractionCooldownEndTime;                        // 0x0048(0x0008)
};

// ScriptStruct MassSmartObjects.MassSmartObjectTimedBehaviorFragment
// 0x0004 (0x0004 - 0x0000)
struct FMassSmartObjectTimedBehaviorFragment final : public FMassFragment
{
public:
	float                                         UseTime;                                           // 0x0000(0x0004)
};

// ScriptStruct MassSmartObjects.MassInActiveSmartObjectsRangeTag
// 0x0000 (0x0001 - 0x0001)
struct FMassInActiveSmartObjectsRangeTag final : public FMassTag
{
};

// ScriptStruct MassSmartObjects.SmartObjectRegistrationFragment
// 0x0010 (0x0010 - 0x0000)
struct FSmartObjectRegistrationFragment final : public FMassFragment
{
public:
	TWeakObjectPtr<class USmartObjectDefinition>  Asset;                                             // 0x0000(0x0008)
	struct FSmartObjectHandle                     Handle;                                            // 0x0008(0x0008)
};

// ScriptStruct MassSmartObjects.SmartObjectCandidateSlot
// 0x0020 (0x0020 - 0x0000)
struct FSmartObjectCandidateSlot final
{
public:
	struct FSmartObjectRequestResult              Result;                                            // 0x0000(0x0018)
	float                                         Cost;                                              // 0x0018(0x0004)
	uint8                                         Pad_1C[0x4];                                       // 0x001C(0x0004)
};

// ScriptStruct MassSmartObjects.MassSmartObjectRequestID
// 0x0008 (0x0008 - 0x0000)
struct FMassSmartObjectRequestID final
{
public:
	struct FMassEntityHandle                      Entity;                                            // 0x0000(0x0008)
};

// ScriptStruct MassSmartObjects.MassSmartObjectCandidateSlots
// 0x0088 (0x0088 - 0x0000)
struct FMassSmartObjectCandidateSlots final
{
public:
	uint8                                         Pad_0[0x80];                                       // 0x0000(0x0080)
	uint8                                         NumSlots;                                          // 0x0080(0x0001)
	uint8                                         Pad_81[0x7];                                       // 0x0081(0x0007)
};

// ScriptStruct MassSmartObjects.MassSmartObjectRequestResultFragment
// 0x0090 (0x0090 - 0x0000)
struct FMassSmartObjectRequestResultFragment final : public FMassFragment
{
public:
	struct FMassSmartObjectCandidateSlots         Candidates;                                        // 0x0000(0x0088)
	bool                                          bProcessed;                                        // 0x0088(0x0001)
	uint8                                         Pad_89[0x7];                                       // 0x0089(0x0007)
};

// ScriptStruct MassSmartObjects.MassSmartObjectWorldLocationRequestFragment
// 0x0088 (0x0088 - 0x0000)
struct FMassSmartObjectWorldLocationRequestFragment final : public FMassFragment
{
public:
	struct FVector                                SearchOrigin;                                      // 0x0000(0x0018)
	struct FMassEntityHandle                      RequestingEntity;                                  // 0x0018(0x0008)
	struct FGameplayTagContainer                  UserTags;                                          // 0x0020(0x0020)
	struct FGameplayTagQuery                      ActivityRequirements;                              // 0x0040(0x0048)
};

// ScriptStruct MassSmartObjects.MassSmartObjectLaneLocationRequestFragment
// 0x0080 (0x0080 - 0x0000)
struct FMassSmartObjectLaneLocationRequestFragment final : public FMassFragment
{
public:
	uint8                                         Pad_0[0x10];                                       // 0x0000(0x0010)
	struct FMassEntityHandle                      RequestingEntity;                                  // 0x0010(0x0008)
	struct FGameplayTagContainer                  UserTags;                                          // 0x0018(0x0020)
	struct FGameplayTagQuery                      ActivityRequirements;                              // 0x0038(0x0048)
};

// ScriptStruct MassSmartObjects.MassSmartObjectCompletedRequestTag
// 0x0000 (0x0001 - 0x0001)
struct FMassSmartObjectCompletedRequestTag final : public FMassTag
{
};

// ScriptStruct MassSmartObjects.SmartObjectMassEntityUserData
// 0x0008 (0x0008 - 0x0000)
struct FSmartObjectMassEntityUserData final
{
public:
	struct FMassEntityHandle                      UserEntity;                                        // 0x0000(0x0008)
};

// ScriptStruct MassSmartObjects.SmartObjectLaneLocation
// 0x0010 (0x0010 - 0x0000)
struct FSmartObjectLaneLocation final
{
public:
	struct FSmartObjectHandle                     ObjectHandle;                                      // 0x0000(0x0008)
	int32                                         LaneIndex;                                         // 0x0008(0x0004)
	float                                         DistanceAlongLane;                                 // 0x000C(0x0004)
};

// ScriptStruct MassSmartObjects.SmartObjectLaneLocationIndices
// 0x0010 (0x0010 - 0x0000)
struct FSmartObjectLaneLocationIndices final
{
public:
	TArray<int32>                                 SmartObjectLaneLocationIndices;                    // 0x0000(0x0010)
};

// ScriptStruct MassSmartObjects.SmartObjectAnnotationData
// 0x00D0 (0x00D0 - 0x0000)
struct FSmartObjectAnnotationData final
{
public:
	struct FZoneGraphDataHandle                   DataHandle;                                        // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	TArray<int32>                                 AffectedLanes;                                     // 0x0008(0x0010)
	TArray<struct FSmartObjectLaneLocation>       SmartObjectLaneLocations;                          // 0x0018(0x0010)
	TMap<struct FSmartObjectHandle, int32>        SmartObjectToLaneLocationIndexLookup;              // 0x0028(0x0050)
	TMap<int32, struct FSmartObjectLaneLocationIndices> LaneToLaneLocationIndicesLookup;             // 0x0078(0x0050)
	uint8                                         Pad_C8[0x8];                                       // 0x00C8(0x0008)
};

}

