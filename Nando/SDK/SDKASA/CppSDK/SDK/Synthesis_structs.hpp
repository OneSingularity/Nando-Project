#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Synthesis

#include "Basic.hpp"

#include "AudioMixer_structs.hpp"
#include "Engine_structs.hpp"
#include "CoreUObject_structs.hpp"
#include "SlateCore_structs.hpp"


namespace SDK
{

// Enum Synthesis.ESynth1OscType
// NumValues: 0x0007
enum class ESynth1OscType : uint8
{
	Sine                                     = 0,
	Saw                                      = 1,
	Triangle                                 = 2,
	Square                                   = 3,
	Noise                                    = 4,
	Count                                    = 5,
	ESynth1OscType_MAX                       = 6,
};

// Enum Synthesis.ESynthLFOType
// NumValues: 0x0009
enum class ESynthLFOType : uint8
{
	Sine                                     = 0,
	UpSaw                                    = 1,
	DownSaw                                  = 2,
	Square                                   = 3,
	Triangle                                 = 4,
	Exponential                              = 5,
	RandomSampleHold                         = 6,
	Count                                    = 7,
	ESynthLFOType_MAX                        = 8,
};

// Enum Synthesis.ESynthLFOMode
// NumValues: 0x0005
enum class ESynthLFOMode : uint8
{
	Sync                                     = 0,
	OneShot                                  = 1,
	Free                                     = 2,
	Count                                    = 3,
	ESynthLFOMode_MAX                        = 4,
};

// Enum Synthesis.ESynthLFOPatchType
// NumValues: 0x000B
enum class ESynthLFOPatchType : uint8
{
	PatchToNone                              = 0,
	PatchToGain                              = 1,
	PatchToOscFreq                           = 2,
	PatchToFilterFreq                        = 3,
	PatchToFilterQ                           = 4,
	PatchToOscPulseWidth                     = 5,
	PatchToOscPan                            = 6,
	PatchLFO1ToLFO2Frequency                 = 7,
	PatchLFO1ToLFO2Gain                      = 8,
	Count                                    = 9,
	ESynthLFOPatchType_MAX                   = 10,
};

// Enum Synthesis.ESynthModEnvPatch
// NumValues: 0x000A
enum class ESynthModEnvPatch : uint8
{
	PatchToNone                              = 0,
	PatchToOscFreq                           = 1,
	PatchToFilterFreq                        = 2,
	PatchToFilterQ                           = 3,
	PatchToLFO1Gain                          = 4,
	PatchToLFO2Gain                          = 5,
	PatchToLFO1Freq                          = 6,
	PatchToLFO2Freq                          = 7,
	Count                                    = 8,
	ESynthModEnvPatch_MAX                    = 9,
};

// Enum Synthesis.ESynthModEnvBiasPatch
// NumValues: 0x000A
enum class ESynthModEnvBiasPatch : uint8
{
	PatchToNone                              = 0,
	PatchToOscFreq                           = 1,
	PatchToFilterFreq                        = 2,
	PatchToFilterQ                           = 3,
	PatchToLFO1Gain                          = 4,
	PatchToLFO2Gain                          = 5,
	PatchToLFO1Freq                          = 6,
	PatchToLFO2Freq                          = 7,
	Count                                    = 8,
	ESynthModEnvBiasPatch_MAX                = 9,
};

// Enum Synthesis.ESynthFilterType
// NumValues: 0x0006
enum class ESynthFilterType : uint8
{
	LowPass                                  = 0,
	HighPass                                 = 1,
	BandPass                                 = 2,
	BandStop                                 = 3,
	Count                                    = 4,
	ESynthFilterType_MAX                     = 5,
};

// Enum Synthesis.ESynthFilterAlgorithm
// NumValues: 0x0005
enum class ESynthFilterAlgorithm : uint8
{
	OnePole                                  = 0,
	StateVariable                            = 1,
	Ladder                                   = 2,
	Count                                    = 3,
	ESynthFilterAlgorithm_MAX                = 4,
};

// Enum Synthesis.ESynthStereoDelayMode
// NumValues: 0x0005
enum class ESynthStereoDelayMode : uint8
{
	Normal                                   = 0,
	Cross                                    = 1,
	PingPong                                 = 2,
	Count                                    = 3,
	ESynthStereoDelayMode_MAX                = 4,
};

// Enum Synthesis.ESynth1PatchSource
// NumValues: 0x0006
enum class ESynth1PatchSource : uint8
{
	LFO1                                     = 0,
	LFO2                                     = 1,
	Envelope                                 = 2,
	BiasEnvelope                             = 3,
	Count                                    = 4,
	ESynth1PatchSource_MAX                   = 5,
};

// Enum Synthesis.ESynth1PatchDestination
// NumValues: 0x0010
enum class ESynth1PatchDestination : uint8
{
	Osc1Gain                                 = 0,
	Osc1Frequency                            = 1,
	Osc1Pulsewidth                           = 2,
	Osc2Gain                                 = 3,
	Osc2Frequency                            = 4,
	Osc2Pulsewidth                           = 5,
	FilterFrequency                          = 6,
	FilterQ                                  = 7,
	Gain                                     = 8,
	Pan                                      = 9,
	LFO1Frequency                            = 10,
	LFO1Gain                                 = 11,
	LFO2Frequency                            = 12,
	LFO2Gain                                 = 13,
	Count                                    = 14,
	ESynth1PatchDestination_MAX              = 15,
};

// Enum Synthesis.ESubmixEffectConvolutionReverbBlockSize
// NumValues: 0x0004
enum class ESubmixEffectConvolutionReverbBlockSize : uint8
{
	BlockSize256                             = 0,
	BlockSize512                             = 1,
	BlockSize1024                            = 2,
	ESubmixEffectConvolutionReverbBlockSize_MAX = 3,
};

// Enum Synthesis.ESourceEffectDynamicsProcessorType
// NumValues: 0x0007
enum class ESourceEffectDynamicsProcessorType : uint8
{
	Compressor                               = 0,
	Limiter                                  = 1,
	Expander                                 = 2,
	Gate                                     = 3,
	UpwardsCompressor                        = 4,
	Count                                    = 5,
	ESourceEffectDynamicsProcessorType_MAX   = 6,
};

// Enum Synthesis.ESourceEffectDynamicsPeakMode
// NumValues: 0x0005
enum class ESourceEffectDynamicsPeakMode : uint8
{
	MeanSquared                              = 0,
	RootMeanSquared                          = 1,
	Peak                                     = 2,
	Count                                    = 3,
	ESourceEffectDynamicsPeakMode_MAX        = 4,
};

// Enum Synthesis.EEnvelopeFollowerPeakMode
// NumValues: 0x0005
enum class EEnvelopeFollowerPeakMode : uint8
{
	MeanSquared                              = 0,
	RootMeanSquared                          = 1,
	Peak                                     = 2,
	Count                                    = 3,
	EEnvelopeFollowerPeakMode_MAX            = 4,
};

// Enum Synthesis.ESourceEffectFilterCircuit
// NumValues: 0x0005
enum class ESourceEffectFilterCircuit : uint8
{
	OnePole                                  = 0,
	StateVariable                            = 1,
	Ladder                                   = 2,
	Count                                    = 3,
	ESourceEffectFilterCircuit_MAX           = 4,
};

// Enum Synthesis.ESourceEffectFilterType
// NumValues: 0x0006
enum class ESourceEffectFilterType : uint8
{
	LowPass                                  = 0,
	HighPass                                 = 1,
	BandPass                                 = 2,
	BandStop                                 = 3,
	Count                                    = 4,
	ESourceEffectFilterType_MAX              = 5,
};

// Enum Synthesis.ESourceEffectFilterParam
// NumValues: 0x0004
enum class ESourceEffectFilterParam : uint8
{
	FilterFrequency                          = 0,
	FilterResonance                          = 1,
	Count                                    = 2,
	ESourceEffectFilterParam_MAX             = 3,
};

// Enum Synthesis.EStereoChannelMode
// NumValues: 0x0004
enum class EStereoChannelMode : uint8
{
	MidSide                                  = 0,
	LeftRight                                = 1,
	count                                    = 2,
	EStereoChannelMode_MAX                   = 3,
};

// Enum Synthesis.ESourceEffectMotionFilterModSource
// NumValues: 0x0007
enum class ESourceEffectMotionFilterModSource : uint8
{
	DistanceFromListener                     = 0,
	SpeedRelativeToListener                  = 1,
	SpeedOfSourceEmitter                     = 2,
	SpeedOfListener                          = 3,
	SpeedOfAngleDelta                        = 4,
	Count                                    = 5,
	ESourceEffectMotionFilterModSource_MAX   = 6,
};

// Enum Synthesis.ESourceEffectMotionFilterModDestination
// NumValues: 0x0009
enum class ESourceEffectMotionFilterModDestination : uint8
{
	FilterACutoffFrequency                   = 0,
	FilterAResonance                         = 1,
	FilterAOutputVolumeDB                    = 2,
	FilterBCutoffFrequency                   = 3,
	FilterBResonance                         = 4,
	FilterBOutputVolumeDB                    = 5,
	FilterMix                                = 6,
	Count                                    = 7,
	ESourceEffectMotionFilterModDestination_MAX = 8,
};

// Enum Synthesis.ESourceEffectMotionFilterTopology
// NumValues: 0x0004
enum class ESourceEffectMotionFilterTopology : uint8
{
	SerialMode                               = 0,
	ParallelMode                             = 1,
	Count                                    = 2,
	ESourceEffectMotionFilterTopology_MAX    = 3,
};

// Enum Synthesis.ESourceEffectMotionFilterCircuit
// NumValues: 0x0005
enum class ESourceEffectMotionFilterCircuit : uint8
{
	OnePole                                  = 0,
	StateVariable                            = 1,
	Ladder                                   = 2,
	Count                                    = 3,
	ESourceEffectMotionFilterCircuit_MAX     = 4,
};

// Enum Synthesis.ESourceEffectMotionFilterType
// NumValues: 0x0006
enum class ESourceEffectMotionFilterType : uint8
{
	LowPass                                  = 0,
	HighPass                                 = 1,
	BandPass                                 = 2,
	BandStop                                 = 3,
	Count                                    = 4,
	ESourceEffectMotionFilterType_MAX        = 5,
};

// Enum Synthesis.EPhaserLFOType
// NumValues: 0x0009
enum class EPhaserLFOType : uint8
{
	Sine                                     = 0,
	UpSaw                                    = 1,
	DownSaw                                  = 2,
	Square                                   = 3,
	Triangle                                 = 4,
	Exponential                              = 5,
	RandomSampleHold                         = 6,
	Count                                    = 7,
	EPhaserLFOType_MAX                       = 8,
};

// Enum Synthesis.ERingModulatorTypeSourceEffect
// NumValues: 0x0006
enum class ERingModulatorTypeSourceEffect : uint8
{
	Sine                                     = 0,
	Saw                                      = 1,
	Triangle                                 = 2,
	Square                                   = 3,
	Count                                    = 4,
	ERingModulatorTypeSourceEffect_MAX       = 5,
};

// Enum Synthesis.EStereoDelaySourceEffect
// NumValues: 0x0005
enum class EStereoDelaySourceEffect : uint8
{
	Normal                                   = 0,
	Cross                                    = 1,
	PingPong                                 = 2,
	Count                                    = 3,
	EStereoDelaySourceEffect_MAX             = 4,
};

// Enum Synthesis.EStereoDelayFiltertype
// NumValues: 0x0006
enum class EStereoDelayFiltertype : uint8
{
	Lowpass                                  = 0,
	Highpass                                 = 1,
	Bandpass                                 = 2,
	Notch                                    = 3,
	Count                                    = 4,
	EStereoDelayFiltertype_MAX               = 5,
};

// Enum Synthesis.ESubmixFilterType
// NumValues: 0x0006
enum class ESubmixFilterType : uint8
{
	LowPass                                  = 0,
	HighPass                                 = 1,
	BandPass                                 = 2,
	BandStop                                 = 3,
	Count                                    = 4,
	ESubmixFilterType_MAX                    = 5,
};

// Enum Synthesis.ESubmixFilterAlgorithm
// NumValues: 0x0005
enum class ESubmixFilterAlgorithm : uint8
{
	OnePole                                  = 0,
	StateVariable                            = 1,
	Ladder                                   = 2,
	Count                                    = 3,
	ESubmixFilterAlgorithm_MAX               = 4,
};

// Enum Synthesis.ETapLineMode
// NumValues: 0x0004
enum class ETapLineMode : uint8
{
	SendToChannel                            = 0,
	Panning                                  = 1,
	Disabled                                 = 2,
	ETapLineMode_MAX                         = 3,
};

// Enum Synthesis.EGranularSynthEnvelopeType
// NumValues: 0x0010
enum class EGranularSynthEnvelopeType : uint8
{
	Rectangular                              = 0,
	Triangle                                 = 1,
	DownwardTriangle                         = 2,
	UpwardTriangle                           = 3,
	ExponentialDecay                         = 4,
	ExponentialIncrease                      = 5,
	Gaussian                                 = 6,
	Hanning                                  = 7,
	Lanczos                                  = 8,
	Cosine                                   = 9,
	CosineSquared                            = 10,
	Welch                                    = 11,
	Blackman                                 = 12,
	BlackmanHarris                           = 13,
	Count                                    = 14,
	EGranularSynthEnvelopeType_MAX           = 15,
};

// Enum Synthesis.EGranularSynthSeekType
// NumValues: 0x0004
enum class EGranularSynthSeekType : uint8
{
	FromBeginning                            = 0,
	FromCurrentPosition                      = 1,
	Count                                    = 2,
	EGranularSynthSeekType_MAX               = 3,
};

// Enum Synthesis.CurveInterpolationType
// NumValues: 0x0004
enum class ECurveInterpolationType : uint8
{
	AUTOINTERP                               = 0,
	LINEAR                                   = 1,
	CONSTANT                                 = 2,
	CurveInterpolationType_MAX               = 3,
};

// Enum Synthesis.ESamplePlayerSeekType
// NumValues: 0x0005
enum class ESamplePlayerSeekType : uint8
{
	FromBeginning                            = 0,
	FromCurrentPosition                      = 1,
	FromEnd                                  = 2,
	Count                                    = 3,
	ESamplePlayerSeekType_MAX                = 4,
};

// Enum Synthesis.ESynthKnobSize
// NumValues: 0x0004
enum class ESynthKnobSize : uint8
{
	Medium                                   = 0,
	Large                                    = 1,
	Count                                    = 2,
	ESynthKnobSize_MAX                       = 3,
};

// Enum Synthesis.ESynthSlateSizeType
// NumValues: 0x0005
enum class ESynthSlateSizeType : uint8
{
	Small                                    = 0,
	Medium                                   = 1,
	Large                                    = 2,
	Count                                    = 3,
	ESynthSlateSizeType_MAX                  = 4,
};

// Enum Synthesis.ESynthSlateColorStyle
// NumValues: 0x0004
enum class ESynthSlateColorStyle : uint8
{
	Light                                    = 0,
	Dark                                     = 1,
	Count                                    = 2,
	ESynthSlateColorStyle_MAX                = 3,
};

// ScriptStruct Synthesis.Synth1PatchCable
// 0x0008 (0x0008 - 0x0000)
struct FSynth1PatchCable final
{
public:
	float                                         Depth;                                             // 0x0000(0x0004)
	ESynth1PatchDestination                       destination;                                       // 0x0004(0x0001)
	uint8                                         Pad_5[0x3];                                        // 0x0005(0x0003)
};

// ScriptStruct Synthesis.PatchId
// 0x0004 (0x0004 - 0x0000)
struct FPatchId final
{
public:
	int32                                         ID;                                                // 0x0000(0x0004)
};

// ScriptStruct Synthesis.EpicSynth1Patch
// 0x0018 (0x0018 - 0x0000)
struct FEpicSynth1Patch final
{
public:
	ESynth1PatchSource                            PatchSource;                                       // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	TArray<struct FSynth1PatchCable>              PatchCables;                                       // 0x0008(0x0010)
};

// ScriptStruct Synthesis.ModularSynthPreset
// 0x00D8 (0x00E0 - 0x0008)
struct FModularSynthPreset final : public FTableRowBase
{
public:
	uint8                                         bEnablePolyphony : 1;                              // 0x0008(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         Pad_9[0x3];                                        // 0x0009(0x0003)
	ESynth1OscType                                Osc1Type;                                          // 0x000C(0x0001)
	uint8                                         Pad_D[0x3];                                        // 0x000D(0x0003)
	float                                         Osc1Gain;                                          // 0x0010(0x0004)
	float                                         Osc1Octave;                                        // 0x0014(0x0004)
	float                                         Osc1Semitones;                                     // 0x0018(0x0004)
	float                                         Osc1Cents;                                         // 0x001C(0x0004)
	float                                         Osc1PulseWidth;                                    // 0x0020(0x0004)
	ESynth1OscType                                Osc2Type;                                          // 0x0024(0x0001)
	uint8                                         Pad_25[0x3];                                       // 0x0025(0x0003)
	float                                         Osc2Gain;                                          // 0x0028(0x0004)
	float                                         Osc2Octave;                                        // 0x002C(0x0004)
	float                                         Osc2Semitones;                                     // 0x0030(0x0004)
	float                                         Osc2Cents;                                         // 0x0034(0x0004)
	float                                         Osc2PulseWidth;                                    // 0x0038(0x0004)
	float                                         Portamento;                                        // 0x003C(0x0004)
	uint8                                         bEnableUnison : 1;                                 // 0x0040(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         bEnableOscillatorSync : 1;                         // 0x0040(0x0001)(BitIndex: 0x01, PropSize: 0x0001)
	uint8                                         Pad_41[0x3];                                       // 0x0041(0x0003)
	float                                         Spread;                                            // 0x0044(0x0004)
	float                                         Pan;                                               // 0x0048(0x0004)
	float                                         LFO1Frequency;                                     // 0x004C(0x0004)
	float                                         LFO1Gain;                                          // 0x0050(0x0004)
	ESynthLFOType                                 LFO1Type;                                          // 0x0054(0x0001)
	ESynthLFOMode                                 LFO1Mode;                                          // 0x0055(0x0001)
	ESynthLFOPatchType                            LFO1PatchType;                                     // 0x0056(0x0001)
	uint8                                         Pad_57[0x1];                                       // 0x0057(0x0001)
	float                                         LFO2Frequency;                                     // 0x0058(0x0004)
	float                                         LFO2Gain;                                          // 0x005C(0x0004)
	ESynthLFOType                                 LFO2Type;                                          // 0x0060(0x0001)
	ESynthLFOMode                                 LFO2Mode;                                          // 0x0061(0x0001)
	ESynthLFOPatchType                            LFO2PatchType;                                     // 0x0062(0x0001)
	uint8                                         Pad_63[0x1];                                       // 0x0063(0x0001)
	float                                         GainDb;                                            // 0x0064(0x0004)
	float                                         AttackTime;                                        // 0x0068(0x0004)
	float                                         DecayTime;                                         // 0x006C(0x0004)
	float                                         SustainGain;                                       // 0x0070(0x0004)
	float                                         ReleaseTime;                                       // 0x0074(0x0004)
	ESynthModEnvPatch                             ModEnvPatchType;                                   // 0x0078(0x0001)
	ESynthModEnvBiasPatch                         ModEnvBiasPatchType;                               // 0x0079(0x0001)
	uint8                                         Pad_7A[0x2];                                       // 0x007A(0x0002)
	uint8                                         bInvertModulationEnvelope : 1;                     // 0x007C(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         bInvertModulationEnvelopeBias : 1;                 // 0x007C(0x0001)(BitIndex: 0x01, PropSize: 0x0001)
	uint8                                         Pad_7D[0x3];                                       // 0x007D(0x0003)
	float                                         ModulationEnvelopeDepth;                           // 0x0080(0x0004)
	float                                         ModulationEnvelopeAttackTime;                      // 0x0084(0x0004)
	float                                         ModulationEnvelopeDecayTime;                       // 0x0088(0x0004)
	float                                         ModulationEnvelopeSustainGain;                     // 0x008C(0x0004)
	float                                         ModulationEnvelopeReleaseTime;                     // 0x0090(0x0004)
	uint8                                         bLegato : 1;                                       // 0x0094(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         bRetrigger : 1;                                    // 0x0094(0x0001)(BitIndex: 0x01, PropSize: 0x0001)
	uint8                                         Pad_95[0x3];                                       // 0x0095(0x0003)
	float                                         FilterFrequency;                                   // 0x0098(0x0004)
	float                                         FilterQ;                                           // 0x009C(0x0004)
	ESynthFilterType                              FilterType;                                        // 0x00A0(0x0001)
	ESynthFilterAlgorithm                         FilterAlgorithm;                                   // 0x00A1(0x0001)
	uint8                                         Pad_A2[0x2];                                       // 0x00A2(0x0002)
	uint8                                         bStereoDelayEnabled : 1;                           // 0x00A4(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         Pad_A5[0x3];                                       // 0x00A5(0x0003)
	ESynthStereoDelayMode                         StereoDelayMode;                                   // 0x00A8(0x0001)
	uint8                                         Pad_A9[0x3];                                       // 0x00A9(0x0003)
	float                                         StereoDelayTime;                                   // 0x00AC(0x0004)
	float                                         StereoDelayFeedback;                               // 0x00B0(0x0004)
	float                                         StereoDelayWetlevel;                               // 0x00B4(0x0004)
	float                                         StereoDelayRatio;                                  // 0x00B8(0x0004)
	uint8                                         bChorusEnabled : 1;                                // 0x00BC(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         Pad_BD[0x3];                                       // 0x00BD(0x0003)
	float                                         ChorusDepth;                                       // 0x00C0(0x0004)
	float                                         ChorusFeedback;                                    // 0x00C4(0x0004)
	float                                         ChorusFrequency;                                   // 0x00C8(0x0004)
	uint8                                         Pad_CC[0x4];                                       // 0x00CC(0x0004)
	TArray<struct FEpicSynth1Patch>               Patches;                                           // 0x00D0(0x0010)
};

// ScriptStruct Synthesis.ModularSynthPresetBankEntry
// 0x00F0 (0x00F0 - 0x0000)
struct FModularSynthPresetBankEntry final
{
public:
	class FString                                 PresetName;                                        // 0x0000(0x0010)
	struct FModularSynthPreset                    Preset;                                            // 0x0010(0x00E0)
};

// ScriptStruct Synthesis.SourceEffectBitCrusherBaseSettings
// 0x0008 (0x0008 - 0x0000)
struct FSourceEffectBitCrusherBaseSettings final
{
public:
	float                                         SampleRate;                                        // 0x0000(0x0004)
	float                                         BitDepth;                                          // 0x0004(0x0004)
};

// ScriptStruct Synthesis.SourceEffectBitCrusherSettings
// 0x00C0 (0x00C0 - 0x0000)
struct FSourceEffectBitCrusherSettings final
{
public:
	float                                         CrushedSampleRate;                                 // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FSoundModulationDestinationSettings    SampleRateModulation;                              // 0x0008(0x0058)
	float                                         CrushedBits;                                       // 0x0060(0x0004)
	uint8                                         Pad_64[0x4];                                       // 0x0064(0x0004)
	struct FSoundModulationDestinationSettings    BitModulation;                                     // 0x0068(0x0058)
};

// ScriptStruct Synthesis.SourceEffectChorusBaseSettings
// 0x0018 (0x0018 - 0x0000)
struct FSourceEffectChorusBaseSettings final
{
public:
	float                                         Depth;                                             // 0x0000(0x0004)
	float                                         Frequency;                                         // 0x0004(0x0004)
	float                                         Feedback;                                          // 0x0008(0x0004)
	float                                         WetLevel;                                          // 0x000C(0x0004)
	float                                         DryLevel;                                          // 0x0010(0x0004)
	float                                         Spread;                                            // 0x0014(0x0004)
};

// ScriptStruct Synthesis.SourceEffectChorusSettings
// 0x0228 (0x0228 - 0x0000)
struct FSourceEffectChorusSettings final
{
public:
	float                                         Depth;                                             // 0x0000(0x0004)
	float                                         Frequency;                                         // 0x0004(0x0004)
	float                                         Feedback;                                          // 0x0008(0x0004)
	float                                         WetLevel;                                          // 0x000C(0x0004)
	float                                         DryLevel;                                          // 0x0010(0x0004)
	float                                         Spread;                                            // 0x0014(0x0004)
	struct FSoundModulationDestinationSettings    DepthModulation;                                   // 0x0018(0x0058)
	struct FSoundModulationDestinationSettings    FrequencyModulation;                               // 0x0070(0x0058)
	struct FSoundModulationDestinationSettings    FeedbackModulation;                                // 0x00C8(0x0058)
	struct FSoundModulationDestinationSettings    WetModulation;                                     // 0x0120(0x0058)
	struct FSoundModulationDestinationSettings    DryModulation;                                     // 0x0178(0x0058)
	struct FSoundModulationDestinationSettings    SpreadModulation;                                  // 0x01D0(0x0058)
};

// ScriptStruct Synthesis.SourceEffectConvolutionReverbSettings
// 0x0010 (0x0010 - 0x0000)
struct FSourceEffectConvolutionReverbSettings final
{
public:
	float                                         NormalizationVolumeDb;                             // 0x0000(0x0004)
	float                                         WetVolumeDb;                                       // 0x0004(0x0004)
	float                                         DryVolumeDb;                                       // 0x0008(0x0004)
	bool                                          bBypass;                                           // 0x000C(0x0001)
	uint8                                         Pad_D[0x3];                                        // 0x000D(0x0003)
};

// ScriptStruct Synthesis.SourceEffectDynamicsProcessorSettings
// 0x0028 (0x0028 - 0x0000)
struct FSourceEffectDynamicsProcessorSettings final
{
public:
	ESourceEffectDynamicsProcessorType            DynamicsProcessorType;                             // 0x0000(0x0001)
	ESourceEffectDynamicsPeakMode                 PeakMode;                                          // 0x0001(0x0001)
	uint8                                         Pad_2[0x2];                                        // 0x0002(0x0002)
	float                                         LookAheadMsec;                                     // 0x0004(0x0004)
	float                                         AttackTimeMsec;                                    // 0x0008(0x0004)
	float                                         ReleaseTimeMsec;                                   // 0x000C(0x0004)
	float                                         ThresholdDb;                                       // 0x0010(0x0004)
	float                                         Ratio;                                             // 0x0014(0x0004)
	float                                         KneeBandwidthDb;                                   // 0x0018(0x0004)
	float                                         InputGainDb;                                       // 0x001C(0x0004)
	float                                         OutputGainDb;                                      // 0x0020(0x0004)
	uint8                                         bStereoLinked : 1;                                 // 0x0024(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         bAnalogMode : 1;                                   // 0x0024(0x0001)(BitIndex: 0x01, PropSize: 0x0001)
	uint8                                         Pad_25[0x3];                                       // 0x0025(0x0003)
};

// ScriptStruct Synthesis.SourceEffectEnvelopeFollowerSettings
// 0x000C (0x000C - 0x0000)
struct FSourceEffectEnvelopeFollowerSettings final
{
public:
	float                                         AttackTime;                                        // 0x0000(0x0004)
	float                                         ReleaseTime;                                       // 0x0004(0x0004)
	EEnvelopeFollowerPeakMode                     PeakMode;                                          // 0x0008(0x0001)
	bool                                          bIsAnalogMode;                                     // 0x0009(0x0001)
	uint8                                         Pad_A[0x2];                                        // 0x000A(0x0002)
};

// ScriptStruct Synthesis.SourceEffectEQBand
// 0x0010 (0x0010 - 0x0000)
struct FSourceEffectEQBand final
{
public:
	float                                         Frequency;                                         // 0x0000(0x0004)
	float                                         Bandwidth;                                         // 0x0004(0x0004)
	float                                         GainDb;                                            // 0x0008(0x0004)
	uint8                                         bEnabled : 1;                                      // 0x000C(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         Pad_D[0x3];                                        // 0x000D(0x0003)
};

// ScriptStruct Synthesis.SourceEffectEQSettings
// 0x0010 (0x0010 - 0x0000)
struct FSourceEffectEQSettings final
{
public:
	TArray<struct FSourceEffectEQBand>            EQBands;                                           // 0x0000(0x0010)
};

// ScriptStruct Synthesis.SourceEffectFilterAudioBusModulationSettings
// 0x0028 (0x0028 - 0x0000)
struct FSourceEffectFilterAudioBusModulationSettings final
{
public:
	class UAudioBus*                              AudioBus;                                          // 0x0000(0x0008)
	int32                                         EnvelopeFollowerAttackTimeMsec;                    // 0x0008(0x0004)
	int32                                         EnvelopeFollowerReleaseTimeMsec;                   // 0x000C(0x0004)
	float                                         EnvelopeGainMultiplier;                            // 0x0010(0x0004)
	ESourceEffectFilterParam                      FilterParam;                                       // 0x0014(0x0001)
	uint8                                         Pad_15[0x3];                                       // 0x0015(0x0003)
	float                                         MinFrequencyModulation;                            // 0x0018(0x0004)
	float                                         MaxFrequencyModulation;                            // 0x001C(0x0004)
	float                                         MinResonanceModulation;                            // 0x0020(0x0004)
	float                                         MaxResonanceModulation;                            // 0x0024(0x0004)
};

// ScriptStruct Synthesis.SourceEffectFilterSettings
// 0x0020 (0x0020 - 0x0000)
struct FSourceEffectFilterSettings final
{
public:
	ESourceEffectFilterCircuit                    FilterCircuit;                                     // 0x0000(0x0001)
	ESourceEffectFilterType                       FilterType;                                        // 0x0001(0x0001)
	uint8                                         Pad_2[0x2];                                        // 0x0002(0x0002)
	float                                         CutoffFrequency;                                   // 0x0004(0x0004)
	float                                         FilterQ;                                           // 0x0008(0x0004)
	uint8                                         Pad_C[0x4];                                        // 0x000C(0x0004)
	TArray<struct FSourceEffectFilterAudioBusModulationSettings> AudioBusModulation;                 // 0x0010(0x0010)
};

// ScriptStruct Synthesis.SourceEffectFoldbackDistortionSettings
// 0x000C (0x000C - 0x0000)
struct FSourceEffectFoldbackDistortionSettings final
{
public:
	float                                         InputGainDb;                                       // 0x0000(0x0004)
	float                                         ThresholdDb;                                       // 0x0004(0x0004)
	float                                         OutputGainDb;                                      // 0x0008(0x0004)
};

// ScriptStruct Synthesis.SourceEffectMidSideSpreaderSettings
// 0x0008 (0x0008 - 0x0000)
struct FSourceEffectMidSideSpreaderSettings final
{
public:
	float                                         SpreadAmount;                                      // 0x0000(0x0004)
	EStereoChannelMode                            InputMode;                                         // 0x0004(0x0001)
	EStereoChannelMode                            OutputMode;                                        // 0x0005(0x0001)
	bool                                          bEqualPower;                                       // 0x0006(0x0001)
	uint8                                         Pad_7[0x1];                                        // 0x0007(0x0001)
};

// ScriptStruct Synthesis.SourceEffectIndividualFilterSettings
// 0x000C (0x000C - 0x0000)
struct FSourceEffectIndividualFilterSettings final
{
public:
	ESourceEffectMotionFilterCircuit              FilterCircuit;                                     // 0x0000(0x0001)
	ESourceEffectMotionFilterType                 FilterType;                                        // 0x0001(0x0001)
	uint8                                         Pad_2[0x2];                                        // 0x0002(0x0002)
	float                                         CutoffFrequency;                                   // 0x0004(0x0004)
	float                                         FilterQ;                                           // 0x0008(0x0004)
};

// ScriptStruct Synthesis.SourceEffectMotionFilterModulationSettings
// 0x0040 (0x0040 - 0x0000)
struct FSourceEffectMotionFilterModulationSettings final
{
public:
	ESourceEffectMotionFilterModSource            ModulationSource;                                  // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector2D                              ModulationInputRange;                              // 0x0008(0x0010)
	struct FVector2D                              ModulationOutputMinimumRange;                      // 0x0018(0x0010)
	struct FVector2D                              ModulationOutputMaximumRange;                      // 0x0028(0x0010)
	float                                         UpdateEaseMS;                                      // 0x0038(0x0004)
	uint8                                         Pad_3C[0x4];                                       // 0x003C(0x0004)
};

// ScriptStruct Synthesis.SourceEffectMotionFilterSettings
// 0x0078 (0x0078 - 0x0000)
struct FSourceEffectMotionFilterSettings final
{
public:
	ESourceEffectMotionFilterTopology             MotionFilterTopology;                              // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         MotionFilterMix;                                   // 0x0004(0x0004)
	struct FSourceEffectIndividualFilterSettings  FilterASettings;                                   // 0x0008(0x000C)
	struct FSourceEffectIndividualFilterSettings  FilterBSettings;                                   // 0x0014(0x000C)
	TMap<ESourceEffectMotionFilterModDestination, struct FSourceEffectMotionFilterModulationSettings> ModulationMappings; // 0x0020(0x0050)
	float                                         DryVolumeDb;                                       // 0x0070(0x0004)
	uint8                                         Pad_74[0x4];                                       // 0x0074(0x0004)
};

// ScriptStruct Synthesis.SourceEffectPannerSettings
// 0x0008 (0x0008 - 0x0000)
struct FSourceEffectPannerSettings final
{
public:
	float                                         Spread;                                            // 0x0000(0x0004)
	float                                         Pan;                                               // 0x0004(0x0004)
};

// ScriptStruct Synthesis.SourceEffectPhaserSettings
// 0x0010 (0x0010 - 0x0000)
struct FSourceEffectPhaserSettings final
{
public:
	float                                         WetLevel;                                          // 0x0000(0x0004)
	float                                         Frequency;                                         // 0x0004(0x0004)
	float                                         Feedback;                                          // 0x0008(0x0004)
	EPhaserLFOType                                LFOType;                                           // 0x000C(0x0001)
	bool                                          UseQuadraturePhase;                                // 0x000D(0x0001)
	uint8                                         Pad_E[0x2];                                        // 0x000E(0x0002)
};

// ScriptStruct Synthesis.SourceEffectRingModulationSettings
// 0x0020 (0x0020 - 0x0000)
struct FSourceEffectRingModulationSettings final
{
public:
	ERingModulatorTypeSourceEffect                ModulatorType;                                     // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         Frequency;                                         // 0x0004(0x0004)
	float                                         Depth;                                             // 0x0008(0x0004)
	float                                         DryLevel;                                          // 0x000C(0x0004)
	float                                         WetLevel;                                          // 0x0010(0x0004)
	uint8                                         Pad_14[0x4];                                       // 0x0014(0x0004)
	class UAudioBus*                              AudioBusModulator;                                 // 0x0018(0x0008)
};

// ScriptStruct Synthesis.SourceEffectSimpleDelaySettings
// 0x0018 (0x0018 - 0x0000)
struct FSourceEffectSimpleDelaySettings final
{
public:
	float                                         SpeedOfSound;                                      // 0x0000(0x0004)
	float                                         DelayAmount;                                       // 0x0004(0x0004)
	float                                         DryAmount;                                         // 0x0008(0x0004)
	float                                         WetAmount;                                         // 0x000C(0x0004)
	float                                         Feedback;                                          // 0x0010(0x0004)
	uint8                                         bDelayBasedOnDistance : 1;                         // 0x0014(0x0001)(BitIndex: 0x00, PropSize: 0x0001)
	uint8                                         bUseDistanceOverride : 1;                          // 0x0014(0x0001)(BitIndex: 0x01, PropSize: 0x0001)
	uint8                                         Pad_15[0x3];                                       // 0x0015(0x0003)
};

// ScriptStruct Synthesis.SourceEffectStereoDelaySettings
// 0x0024 (0x0024 - 0x0000)
struct FSourceEffectStereoDelaySettings final
{
public:
	EStereoDelaySourceEffect                      DelayMode;                                         // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         DelayTimeMsec;                                     // 0x0004(0x0004)
	float                                         Feedback;                                          // 0x0008(0x0004)
	float                                         DelayRatio;                                        // 0x000C(0x0004)
	float                                         WetLevel;                                          // 0x0010(0x0004)
	float                                         DryLevel;                                          // 0x0014(0x0004)
	bool                                          bFilterEnabled;                                    // 0x0018(0x0001)
	EStereoDelayFiltertype                        FilterType;                                        // 0x0019(0x0001)
	uint8                                         Pad_1A[0x2];                                       // 0x001A(0x0002)
	float                                         FilterFrequency;                                   // 0x001C(0x0004)
	float                                         FilterQ;                                           // 0x0020(0x0004)
};

// ScriptStruct Synthesis.SourceEffectWaveShaperSettings
// 0x0008 (0x0008 - 0x0000)
struct FSourceEffectWaveShaperSettings final
{
public:
	float                                         Amount;                                            // 0x0000(0x0004)
	float                                         OutputGainDb;                                      // 0x0004(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectConvolutionReverbSettings
// 0x0030 (0x0030 - 0x0000)
struct FSubmixEffectConvolutionReverbSettings final
{
public:
	float                                         NormalizationVolumeDb;                             // 0x0000(0x0004)
	float                                         WetVolumeDb;                                       // 0x0004(0x0004)
	float                                         DryVolumeDb;                                       // 0x0008(0x0004)
	bool                                          bBypass;                                           // 0x000C(0x0001)
	bool                                          bMixInputChannelFormatToImpulseResponseFormat;     // 0x000D(0x0001)
	bool                                          bMixReverbOutputToOutputChannelFormat;             // 0x000E(0x0001)
	uint8                                         Pad_F[0x1];                                        // 0x000F(0x0001)
	float                                         SurroundRearChannelBleedDb;                        // 0x0010(0x0004)
	bool                                          bInvertRearChannelBleedPhase;                      // 0x0014(0x0001)
	bool                                          bSurroundRearChannelFlip;                          // 0x0015(0x0001)
	uint8                                         Pad_16[0x2];                                       // 0x0016(0x0002)
	float                                         SurroundRearChannelBleedAmount;                    // 0x0018(0x0004)
	uint8                                         Pad_1C[0x4];                                       // 0x001C(0x0004)
	class UAudioImpulseResponse*                  ImpulseResponse;                                   // 0x0020(0x0008)
	bool                                          AllowHardwareAcceleration;                         // 0x0028(0x0001)
	uint8                                         Pad_29[0x7];                                       // 0x0029(0x0007)
};

// ScriptStruct Synthesis.SubmixEffectDelaySettings
// 0x000C (0x000C - 0x0000)
struct FSubmixEffectDelaySettings final
{
public:
	float                                         MaximumDelayLength;                                // 0x0000(0x0004)
	float                                         InterpolationTime;                                 // 0x0004(0x0004)
	float                                         DelayLength;                                       // 0x0008(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectFilterSettings
// 0x000C (0x000C - 0x0000)
struct FSubmixEffectFilterSettings final
{
public:
	ESubmixFilterType                             FilterType;                                        // 0x0000(0x0001)
	ESubmixFilterAlgorithm                        FilterAlgorithm;                                   // 0x0001(0x0001)
	uint8                                         Pad_2[0x2];                                        // 0x0002(0x0002)
	float                                         FilterFrequency;                                   // 0x0004(0x0004)
	float                                         FilterQ;                                           // 0x0008(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectFlexiverbSettings
// 0x0010 (0x0010 - 0x0000)
struct FSubmixEffectFlexiverbSettings final
{
public:
	float                                         PreDelay;                                          // 0x0000(0x0004)
	float                                         DecayTime;                                         // 0x0004(0x0004)
	float                                         RoomDampening;                                     // 0x0008(0x0004)
	int32                                         Complexity;                                        // 0x000C(0x0004)
};

// ScriptStruct Synthesis.DynamicsBandSettings
// 0x0020 (0x0020 - 0x0000)
struct FDynamicsBandSettings final
{
public:
	float                                         CrossoverTopFrequency;                             // 0x0000(0x0004)
	float                                         AttackTimeMsec;                                    // 0x0004(0x0004)
	float                                         ReleaseTimeMsec;                                   // 0x0008(0x0004)
	float                                         ThresholdDb;                                       // 0x000C(0x0004)
	float                                         Ratio;                                             // 0x0010(0x0004)
	float                                         KneeBandwidthDb;                                   // 0x0014(0x0004)
	float                                         InputGainDb;                                       // 0x0018(0x0004)
	float                                         OutputGainDb;                                      // 0x001C(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectMultibandCompressorSettings
// 0x0038 (0x0038 - 0x0000)
struct FSubmixEffectMultibandCompressorSettings final
{
public:
	ESubmixEffectDynamicsProcessorType            DynamicsProcessorType;                             // 0x0000(0x0001)
	ESubmixEffectDynamicsPeakMode                 PeakMode;                                          // 0x0001(0x0001)
	ESubmixEffectDynamicsChannelLinkMode          LinkMode;                                          // 0x0002(0x0001)
	uint8                                         Pad_3[0x1];                                        // 0x0003(0x0001)
	float                                         LookAheadMsec;                                     // 0x0004(0x0004)
	bool                                          bAnalogMode;                                       // 0x0008(0x0001)
	bool                                          bFourPole;                                         // 0x0009(0x0001)
	bool                                          bBypass;                                           // 0x000A(0x0001)
	ESubmixEffectDynamicsKeySource                KeySource;                                         // 0x000B(0x0001)
	uint8                                         Pad_C[0x4];                                        // 0x000C(0x0004)
	class UAudioBus*                              ExternalAudioBus;                                  // 0x0010(0x0008)
	class USoundSubmix*                           ExternalSubmix;                                    // 0x0018(0x0008)
	float                                         KeyGainDb;                                         // 0x0020(0x0004)
	bool                                          bKeyAudition;                                      // 0x0024(0x0001)
	uint8                                         Pad_25[0x3];                                       // 0x0025(0x0003)
	TArray<struct FDynamicsBandSettings>          Bands;                                             // 0x0028(0x0010)
};

// ScriptStruct Synthesis.SubmixEffectStereoDelaySettings
// 0x0024 (0x0024 - 0x0000)
struct FSubmixEffectStereoDelaySettings final
{
public:
	EStereoDelaySourceEffect                      DelayMode;                                         // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         DelayTimeMsec;                                     // 0x0004(0x0004)
	float                                         Feedback;                                          // 0x0008(0x0004)
	float                                         DelayRatio;                                        // 0x000C(0x0004)
	float                                         WetLevel;                                          // 0x0010(0x0004)
	float                                         DryLevel;                                          // 0x0014(0x0004)
	bool                                          bFilterEnabled;                                    // 0x0018(0x0001)
	EStereoDelayFiltertype                        FilterType;                                        // 0x0019(0x0001)
	uint8                                         Pad_1A[0x2];                                       // 0x001A(0x0002)
	float                                         FilterFrequency;                                   // 0x001C(0x0004)
	float                                         FilterQ;                                           // 0x0020(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectStereoToQuadSettings
// 0x0008 (0x0008 - 0x0000)
struct FSubmixEffectStereoToQuadSettings final
{
public:
	bool                                          bFlipChannels;                                     // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         RearChannelGain;                                   // 0x0004(0x0004)
};

// ScriptStruct Synthesis.TapDelayInfo
// 0x0018 (0x0018 - 0x0000)
struct FTapDelayInfo final
{
public:
	ETapLineMode                                  TapLineMode;                                       // 0x0000(0x0001)
	uint8                                         Pad_1[0x3];                                        // 0x0001(0x0003)
	float                                         DelayLength;                                       // 0x0004(0x0004)
	float                                         Gain;                                              // 0x0008(0x0004)
	int32                                         OutputChannel;                                     // 0x000C(0x0004)
	float                                         PanInDegrees;                                      // 0x0010(0x0004)
	int32                                         TapId;                                             // 0x0014(0x0004)
};

// ScriptStruct Synthesis.SubmixEffectTapDelaySettings
// 0x0018 (0x0018 - 0x0000)
struct FSubmixEffectTapDelaySettings final
{
public:
	float                                         MaximumDelayLength;                                // 0x0000(0x0004)
	float                                         InterpolationTime;                                 // 0x0004(0x0004)
	TArray<struct FTapDelayInfo>                  Taps;                                              // 0x0008(0x0010)
};

// ScriptStruct Synthesis.Synth2DSliderStyle
// 0x0388 (0x0390 - 0x0008)
struct FSynth2DSliderStyle final : public FSlateWidgetStyle
{
public:
	uint8                                         Pad_8[0x8];                                        // 0x0008(0x0008)
	struct FSlateBrush                            NormalThumbImage;                                  // 0x0010(0x00B0)
	struct FSlateBrush                            DisabledThumbImage;                                // 0x00C0(0x00B0)
	struct FSlateBrush                            NormalBarImage;                                    // 0x0170(0x00B0)
	struct FSlateBrush                            DisabledBarImage;                                  // 0x0220(0x00B0)
	struct FSlateBrush                            BackgroundImage;                                   // 0x02D0(0x00B0)
	float                                         BarThickness;                                      // 0x0380(0x0004)
	uint8                                         Pad_384[0xC];                                      // 0x0384(0x000C)
};

// ScriptStruct Synthesis.SynthKnobStyle
// 0x02D8 (0x02E0 - 0x0008)
struct FSynthKnobStyle final : public FSlateWidgetStyle
{
public:
	uint8                                         Pad_8[0x8];                                        // 0x0008(0x0008)
	struct FSlateBrush                            LargeKnob;                                         // 0x0010(0x00B0)
	struct FSlateBrush                            LargeKnobOverlay;                                  // 0x00C0(0x00B0)
	struct FSlateBrush                            MediumKnob;                                        // 0x0170(0x00B0)
	struct FSlateBrush                            MediumKnobOverlay;                                 // 0x0220(0x00B0)
	float                                         MinValueAngle;                                     // 0x02D0(0x0004)
	float                                         MaxValueAngle;                                     // 0x02D4(0x0004)
	ESynthKnobSize                                KnobSize;                                          // 0x02D8(0x0001)
	uint8                                         Pad_2D9[0x7];                                      // 0x02D9(0x0007)
};

// ScriptStruct Synthesis.SynthSlateStyle
// 0x0008 (0x0010 - 0x0008)
struct FSynthSlateStyle final : public FSlateWidgetStyle
{
public:
	ESynthSlateSizeType                           SizeType;                                          // 0x0008(0x0001)
	ESynthSlateColorStyle                         ColorStyle;                                        // 0x0009(0x0001)
	uint8                                         Pad_A[0x6];                                        // 0x000A(0x0006)
};

}

