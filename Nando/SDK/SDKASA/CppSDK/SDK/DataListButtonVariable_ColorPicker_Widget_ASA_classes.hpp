#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: DataListButtonVariable_ColorPicker_Widget_ASA

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "CoreUObject_structs.hpp"
#include "DataListButtonVariable_Widget_ASA_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass DataListButtonVariable_ColorPicker_Widget_ASA.DataListButtonVariable_ColorPicker_Widget_ASA_C
// 0x0030 (0x0508 - 0x04D8)
class UDataListButtonVariable_ColorPicker_Widget_ASA_C final : public UDataListButtonVariable_Widget_ASA_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_DataListButtonVariable_ColorPicker_Widget_ASA_C; // 0x04D8(0x0008)
	TMulticastInlineDelegate<void(const struct FLinearColor& Color)> ChangedColor;                   // 0x04E0(0x0010)
	bool                                          bHideColor;                                        // 0x04F0(0x0001)
	uint8                                         Pad_4F1[0x7];                                      // 0x04F1(0x0007)
	TMulticastInlineDelegate<void(bool Toggle)>   SelectedOption;                                    // 0x04F8(0x0010)

public:
	void UpdateColorPicker(const struct FVector2D& ColorPickerSliderValue);
	void SetupTextValue(const class FText& Text, class FText* ReturnText);
	void PreConstruct(bool IsDesignTime);
	void ExtraSetupToggledButton(bool Toggle, bool FromSelectOtherOption);
	void ExecuteUbergraph_DataListButtonVariable_ColorPicker_Widget_ASA(int32 EntryPoint);
	void Construct();
	void BPExecutedCommandForPrimalUI(class FName CommandName, const struct FBPNetExecParams& ExecParams);

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("DataListButtonVariable_ColorPicker_Widget_ASA_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"DataListButtonVariable_ColorPicker_Widget_ASA_C")
	}
	static class UDataListButtonVariable_ColorPicker_Widget_ASA_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UDataListButtonVariable_ColorPicker_Widget_ASA_C>();
	}
};

}

