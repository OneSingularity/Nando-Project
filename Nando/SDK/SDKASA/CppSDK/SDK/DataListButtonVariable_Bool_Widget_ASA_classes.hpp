#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: DataListButtonVariable_Bool_Widget_ASA

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "DataListButtonVariable_Widget_ASA_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass DataListButtonVariable_Bool_Widget_ASA.DataListButtonVariable_Bool_Widget_ASA_C
// 0x0028 (0x0500 - 0x04D8)
class UDataListButtonVariable_Bool_Widget_ASA_C final : public UDataListButtonVariable_Widget_ASA_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_DataListButtonVariable_Bool_Widget_ASA_C; // 0x04D8(0x0008)
	class FString                                 TrueBoolString;                                    // 0x04E0(0x0010)
	class FString                                 FalseBoolString;                                   // 0x04F0(0x0010)

public:
	void SetupTextValue(const class FText& Text, class FText* ReturnText);
	void PreConstruct(bool IsDesignTime);
	void ExtraSetupToggledButton(bool Toggle, bool FromSelectOtherOption);
	void ExecuteUbergraph_DataListButtonVariable_Bool_Widget_ASA(int32 EntryPoint);
	void Construct();

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("DataListButtonVariable_Bool_Widget_ASA_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"DataListButtonVariable_Bool_Widget_ASA_C")
	}
	static class UDataListButtonVariable_Bool_Widget_ASA_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UDataListButtonVariable_Bool_Widget_ASA_C>();
	}
};

}

