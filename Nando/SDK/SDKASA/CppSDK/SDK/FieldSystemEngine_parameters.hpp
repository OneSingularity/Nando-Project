#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: FieldSystemEngine

#include "Basic.hpp"

#include "CoreUObject_structs.hpp"
#include "Chaos_structs.hpp"


namespace SDK::Params
{

// Function FieldSystemEngine.FieldSystemComponent.AddFieldCommand
// 0x0018 (0x0018 - 0x0000)
struct FieldSystemComponent_AddFieldCommand final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	EFieldPhysicsType                             Target;                                            // 0x0001(0x0001)
	uint8                                         Pad_2[0x6];                                        // 0x0002(0x0006)
	class UFieldSystemMetaData*                   MetaData;                                          // 0x0008(0x0008)
	class UFieldNodeBase*                         Field;                                             // 0x0010(0x0008)
};

// Function FieldSystemEngine.FieldSystemComponent.AddPersistentField
// 0x0018 (0x0018 - 0x0000)
struct FieldSystemComponent_AddPersistentField final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	EFieldPhysicsType                             Target;                                            // 0x0001(0x0001)
	uint8                                         Pad_2[0x6];                                        // 0x0002(0x0006)
	class UFieldSystemMetaData*                   MetaData;                                          // 0x0008(0x0008)
	class UFieldNodeBase*                         Field;                                             // 0x0010(0x0008)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyLinearForce
// 0x0028 (0x0028 - 0x0000)
struct FieldSystemComponent_ApplyLinearForce final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                Direction;                                         // 0x0008(0x0018)
	float                                         Magnitude;                                         // 0x0020(0x0004)
	uint8                                         Pad_24[0x4];                                       // 0x0024(0x0004)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyPhysicsField
// 0x0018 (0x0018 - 0x0000)
struct FieldSystemComponent_ApplyPhysicsField final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	EFieldPhysicsType                             Target;                                            // 0x0001(0x0001)
	uint8                                         Pad_2[0x6];                                        // 0x0002(0x0006)
	class UFieldSystemMetaData*                   MetaData;                                          // 0x0008(0x0008)
	class UFieldNodeBase*                         Field;                                             // 0x0010(0x0008)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyRadialForce
// 0x0028 (0x0028 - 0x0000)
struct FieldSystemComponent_ApplyRadialForce final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                position;                                          // 0x0008(0x0018)
	float                                         Magnitude;                                         // 0x0020(0x0004)
	uint8                                         Pad_24[0x4];                                       // 0x0024(0x0004)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyRadialVectorFalloffForce
// 0x0028 (0x0028 - 0x0000)
struct FieldSystemComponent_ApplyRadialVectorFalloffForce final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                position;                                          // 0x0008(0x0018)
	float                                         Radius;                                            // 0x0020(0x0004)
	float                                         Magnitude;                                         // 0x0024(0x0004)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyStayDynamicField
// 0x0028 (0x0028 - 0x0000)
struct FieldSystemComponent_ApplyStayDynamicField final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                position;                                          // 0x0008(0x0018)
	float                                         Radius;                                            // 0x0020(0x0004)
	uint8                                         Pad_24[0x4];                                       // 0x0024(0x0004)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyStrainField
// 0x0030 (0x0030 - 0x0000)
struct FieldSystemComponent_ApplyStrainField final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                position;                                          // 0x0008(0x0018)
	float                                         Radius;                                            // 0x0020(0x0004)
	float                                         Magnitude;                                         // 0x0024(0x0004)
	int32                                         Iterations;                                        // 0x0028(0x0004)
	uint8                                         Pad_2C[0x4];                                       // 0x002C(0x0004)
};

// Function FieldSystemEngine.FieldSystemComponent.ApplyUniformVectorFalloffForce
// 0x0040 (0x0040 - 0x0000)
struct FieldSystemComponent_ApplyUniformVectorFalloffForce final
{
public:
	bool                                          Enabled;                                           // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	struct FVector                                position;                                          // 0x0008(0x0018)
	struct FVector                                Direction;                                         // 0x0020(0x0018)
	float                                         Radius;                                            // 0x0038(0x0004)
	float                                         Magnitude;                                         // 0x003C(0x0004)
};

// Function FieldSystemEngine.FieldSystemMetaDataIteration.SetMetaDataIteration
// 0x0010 (0x0010 - 0x0000)
struct FieldSystemMetaDataIteration_SetMetaDataIteration final
{
public:
	int32                                         Iterations_0;                                      // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	class UFieldSystemMetaDataIteration*          ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.FieldSystemMetaDataProcessingResolution.SetMetaDataaProcessingResolutionType
// 0x0010 (0x0010 - 0x0000)
struct FieldSystemMetaDataProcessingResolution_SetMetaDataaProcessingResolutionType final
{
public:
	EFieldResolutionType                          ResolutionType_0;                                  // 0x0000(0x0001)
	uint8                                         Pad_1[0x7];                                        // 0x0001(0x0007)
	class UFieldSystemMetaDataProcessingResolution* ReturnValue;                                     // 0x0008(0x0008)
};

// Function FieldSystemEngine.FieldSystemMetaDataFilter.SetMetaDataFilterType
// 0x0010 (0x0010 - 0x0000)
struct FieldSystemMetaDataFilter_SetMetaDataFilterType final
{
public:
	EFieldFilterType                              FilterType_0;                                      // 0x0000(0x0001)
	EFieldObjectType                              ObjectType_0;                                      // 0x0001(0x0001)
	EFieldPositionType                            PositionType_0;                                    // 0x0002(0x0001)
	uint8                                         Pad_3[0x5];                                        // 0x0003(0x0005)
	class UFieldSystemMetaDataFilter*             ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.UniformInteger.SetUniformInteger
// 0x0010 (0x0010 - 0x0000)
struct UniformInteger_SetUniformInteger final
{
public:
	int32                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	class UUniformInteger*                        ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.RadialIntMask.SetRadialIntMask
// 0x0038 (0x0038 - 0x0000)
struct RadialIntMask_SetRadialIntMask final
{
public:
	float                                         Radius_0;                                          // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FVector                                position_0;                                        // 0x0008(0x0018)
	int32                                         InteriorValue_0;                                   // 0x0020(0x0004)
	int32                                         ExteriorValue_0;                                   // 0x0024(0x0004)
	ESetMaskConditionType                         SetMaskConditionIn;                                // 0x0028(0x0001)
	uint8                                         Pad_29[0x7];                                       // 0x0029(0x0007)
	class URadialIntMask*                         ReturnValue;                                       // 0x0030(0x0008)
};

// Function FieldSystemEngine.UniformScalar.SetUniformScalar
// 0x0010 (0x0010 - 0x0000)
struct UniformScalar_SetUniformScalar final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	class UUniformScalar*                         ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.WaveScalar.SetWaveScalar
// 0x0038 (0x0038 - 0x0000)
struct WaveScalar_SetWaveScalar final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FVector                                position_0;                                        // 0x0008(0x0018)
	float                                         Wavelength_0;                                      // 0x0020(0x0004)
	float                                         Period_0;                                          // 0x0024(0x0004)
	float                                         Time;                                              // 0x0028(0x0004)
	EWaveFunctionType                             Function_0;                                        // 0x002C(0x0001)
	EFieldFalloffType                             Falloff_0;                                         // 0x002D(0x0001)
	uint8                                         Pad_2E[0x2];                                       // 0x002E(0x0002)
	class UWaveScalar*                            ReturnValue;                                       // 0x0030(0x0008)
};

// Function FieldSystemEngine.RadialFalloff.SetRadialFalloff
// 0x0040 (0x0040 - 0x0000)
struct RadialFalloff_SetRadialFalloff final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	float                                         MinRange_0;                                        // 0x0004(0x0004)
	float                                         MaxRange_0;                                        // 0x0008(0x0004)
	float                                         Default_0;                                         // 0x000C(0x0004)
	float                                         Radius_0;                                          // 0x0010(0x0004)
	uint8                                         Pad_14[0x4];                                       // 0x0014(0x0004)
	struct FVector                                position_0;                                        // 0x0018(0x0018)
	EFieldFalloffType                             Falloff_0;                                         // 0x0030(0x0001)
	uint8                                         Pad_31[0x7];                                       // 0x0031(0x0007)
	class URadialFalloff*                         ReturnValue;                                       // 0x0038(0x0008)
};

// Function FieldSystemEngine.PlaneFalloff.SetPlaneFalloff
// 0x0058 (0x0058 - 0x0000)
struct PlaneFalloff_SetPlaneFalloff final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	float                                         MinRange_0;                                        // 0x0004(0x0004)
	float                                         MaxRange_0;                                        // 0x0008(0x0004)
	float                                         Default_0;                                         // 0x000C(0x0004)
	float                                         Distance_0;                                        // 0x0010(0x0004)
	uint8                                         Pad_14[0x4];                                       // 0x0014(0x0004)
	struct FVector                                position_0;                                        // 0x0018(0x0018)
	struct FVector                                Normal_0;                                          // 0x0030(0x0018)
	EFieldFalloffType                             Falloff_0;                                         // 0x0048(0x0001)
	uint8                                         Pad_49[0x7];                                       // 0x0049(0x0007)
	class UPlaneFalloff*                          ReturnValue;                                       // 0x0050(0x0008)
};

// Function FieldSystemEngine.BoxFalloff.SetBoxFalloff
// 0x0080 (0x0080 - 0x0000)
struct BoxFalloff_SetBoxFalloff final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	float                                         MinRange_0;                                        // 0x0004(0x0004)
	float                                         MaxRange_0;                                        // 0x0008(0x0004)
	float                                         Default_0;                                         // 0x000C(0x0004)
	struct FTransform                             Transform_0;                                       // 0x0010(0x0060)
	EFieldFalloffType                             Falloff_0;                                         // 0x0070(0x0001)
	uint8                                         Pad_71[0x7];                                       // 0x0071(0x0007)
	class UBoxFalloff*                            ReturnValue;                                       // 0x0078(0x0008)
};

// Function FieldSystemEngine.NoiseField.SetNoiseField
// 0x0080 (0x0080 - 0x0000)
struct NoiseField_SetNoiseField final
{
public:
	float                                         MinRange_0;                                        // 0x0000(0x0004)
	float                                         MaxRange_0;                                        // 0x0004(0x0004)
	uint8                                         Pad_8[0x8];                                        // 0x0008(0x0008)
	struct FTransform                             Transform_0;                                       // 0x0010(0x0060)
	class UNoiseField*                            ReturnValue;                                       // 0x0070(0x0008)
	uint8                                         Pad_78[0x8];                                       // 0x0078(0x0008)
};

// Function FieldSystemEngine.UniformVector.SetUniformVector
// 0x0028 (0x0028 - 0x0000)
struct UniformVector_SetUniformVector final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FVector                                Direction_0;                                       // 0x0008(0x0018)
	class UUniformVector*                         ReturnValue;                                       // 0x0020(0x0008)
};

// Function FieldSystemEngine.RadialVector.SetRadialVector
// 0x0028 (0x0028 - 0x0000)
struct RadialVector_SetRadialVector final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	struct FVector                                position_0;                                        // 0x0008(0x0018)
	class URadialVector*                          ReturnValue;                                       // 0x0020(0x0008)
};

// Function FieldSystemEngine.RandomVector.SetRandomVector
// 0x0010 (0x0010 - 0x0000)
struct RandomVector_SetRandomVector final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	class URandomVector*                          ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.OperatorField.SetOperatorField
// 0x0028 (0x0028 - 0x0000)
struct OperatorField_SetOperatorField final
{
public:
	float                                         Magnitude_0;                                       // 0x0000(0x0004)
	uint8                                         Pad_4[0x4];                                        // 0x0004(0x0004)
	const class UFieldNodeBase*                   LeftField_0;                                       // 0x0008(0x0008)
	const class UFieldNodeBase*                   RightField_0;                                      // 0x0010(0x0008)
	EFieldOperationType                           Operation_0;                                       // 0x0018(0x0001)
	uint8                                         Pad_19[0x7];                                       // 0x0019(0x0007)
	class UOperatorField*                         ReturnValue;                                       // 0x0020(0x0008)
};

// Function FieldSystemEngine.ToIntegerField.SetToIntegerField
// 0x0010 (0x0010 - 0x0000)
struct ToIntegerField_SetToIntegerField final
{
public:
	const class UFieldNodeFloat*                  FloatField_0;                                      // 0x0000(0x0008)
	class UToIntegerField*                        ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.ToFloatField.SetToFloatField
// 0x0010 (0x0010 - 0x0000)
struct ToFloatField_SetToFloatField final
{
public:
	const class UFieldNodeInt*                    IntegerField;                                      // 0x0000(0x0008)
	class UToFloatField*                          ReturnValue;                                       // 0x0008(0x0008)
};

// Function FieldSystemEngine.CullingField.SetCullingField
// 0x0020 (0x0020 - 0x0000)
struct CullingField_SetCullingField final
{
public:
	const class UFieldNodeBase*                   Culling_0;                                         // 0x0000(0x0008)
	const class UFieldNodeBase*                   Field_0;                                           // 0x0008(0x0008)
	EFieldCullingOperationType                    Operation_0;                                       // 0x0010(0x0001)
	uint8                                         Pad_11[0x7];                                       // 0x0011(0x0007)
	class UCullingField*                          ReturnValue;                                       // 0x0018(0x0008)
};

// Function FieldSystemEngine.ReturnResultsTerminal.SetReturnResultsTerminal
// 0x0008 (0x0008 - 0x0000)
struct ReturnResultsTerminal_SetReturnResultsTerminal final
{
public:
	class UReturnResultsTerminal*                 ReturnValue;                                       // 0x0000(0x0008)
};

}

