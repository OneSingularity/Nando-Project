#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: ChaosCloth

#include "Basic.hpp"

#include "ChaosCloth_structs.hpp"
#include "ClothingSystemRuntimeCommon_structs.hpp"
#include "ClothingSystemRuntimeCommon_classes.hpp"
#include "CoreUObject_structs.hpp"
#include "ClothingSystemRuntimeInterface_classes.hpp"


namespace SDK
{

// Class ChaosCloth.ChaosClothConfig
// 0x0120 (0x0148 - 0x0028)
class UChaosClothConfig final : public UClothConfigCommon
{
public:
	EClothMassMode                                MassMode;                                          // 0x0028(0x0001)
	uint8                                         Pad_29[0x3];                                       // 0x0029(0x0003)
	float                                         UniformMass;                                       // 0x002C(0x0004)
	float                                         TotalMass;                                         // 0x0030(0x0004)
	float                                         Density;                                           // 0x0034(0x0004)
	float                                         MinPerParticleMass;                                // 0x0038(0x0004)
	struct FChaosClothWeightedValue               EdgeStiffnessWeighted;                             // 0x003C(0x0008)
	struct FChaosClothWeightedValue               BendingStiffnessWeighted;                          // 0x0044(0x0008)
	bool                                          bUseBendingElements;                               // 0x004C(0x0001)
	uint8                                         Pad_4D[0x3];                                       // 0x004D(0x0003)
	float                                         BucklingRatio;                                     // 0x0050(0x0004)
	struct FChaosClothWeightedValue               BucklingStiffnessWeighted;                         // 0x0054(0x0008)
	struct FChaosClothWeightedValue               FlatnessRatio;                                     // 0x005C(0x0008)
	struct FChaosClothWeightedValue               AreaStiffnessWeighted;                             // 0x0064(0x0008)
	float                                         VolumeStiffness;                                   // 0x006C(0x0004)
	struct FChaosClothWeightedValue               TetherStiffness;                                   // 0x0070(0x0008)
	struct FChaosClothWeightedValue               TetherScale;                                       // 0x0078(0x0008)
	bool                                          bUseGeodesicDistance;                              // 0x0080(0x0001)
	uint8                                         Pad_81[0x3];                                       // 0x0081(0x0003)
	float                                         ShapeTargetStiffness;                              // 0x0084(0x0004)
	float                                         CollisionThickness;                                // 0x0088(0x0004)
	float                                         FrictionCoefficient;                               // 0x008C(0x0004)
	bool                                          bUseCCD;                                           // 0x0090(0x0001)
	bool                                          bUseSelfCollisions;                                // 0x0091(0x0001)
	uint8                                         Pad_92[0x2];                                       // 0x0092(0x0002)
	float                                         SelfCollisionThickness;                            // 0x0094(0x0004)
	float                                         SelfCollisionFriction;                             // 0x0098(0x0004)
	bool                                          bUseSelfIntersections;                             // 0x009C(0x0001)
	bool                                          bUseSelfCollisionSpheres;                          // 0x009D(0x0001)
	uint8                                         Pad_9E[0x2];                                       // 0x009E(0x0002)
	float                                         SelfCollisionSphereRadius;                         // 0x00A0(0x0004)
	float                                         SelfCollisionSphereStiffness;                      // 0x00A4(0x0004)
	float                                         SelfCollisionSphereRadiusCullMultiplier;           // 0x00A8(0x0004)
	bool                                          bUseLegacyBackstop;                                // 0x00AC(0x0001)
	uint8                                         Pad_AD[0x3];                                       // 0x00AD(0x0003)
	float                                         DampingCoefficient;                                // 0x00B0(0x0004)
	float                                         LocalDampingCoefficient;                           // 0x00B4(0x0004)
	bool                                          bUsePointBasedWindModel;                           // 0x00B8(0x0001)
	uint8                                         Pad_B9[0x3];                                       // 0x00B9(0x0003)
	struct FChaosClothWeightedValue               Drag;                                              // 0x00BC(0x0008)
	bool                                          bEnableOuterDrag;                                  // 0x00C4(0x0001)
	uint8                                         Pad_C5[0x3];                                       // 0x00C5(0x0003)
	struct FChaosClothWeightedValue               OuterDrag;                                         // 0x00C8(0x0008)
	struct FChaosClothWeightedValue               Lift;                                              // 0x00D0(0x0008)
	bool                                          bEnableOuterLift;                                  // 0x00D8(0x0001)
	uint8                                         Pad_D9[0x3];                                       // 0x00D9(0x0003)
	struct FChaosClothWeightedValue               OuterLift;                                         // 0x00DC(0x0008)
	bool                                          bUseGravityOverride;                               // 0x00E4(0x0001)
	uint8                                         Pad_E5[0x3];                                       // 0x00E5(0x0003)
	float                                         GravityScale;                                      // 0x00E8(0x0004)
	uint8                                         Pad_EC[0x4];                                       // 0x00EC(0x0004)
	struct FVector                                Gravity;                                           // 0x00F0(0x0018)
	struct FChaosClothWeightedValue               Pressure;                                          // 0x0108(0x0008)
	struct FChaosClothWeightedValue               AnimDriveStiffness;                                // 0x0110(0x0008)
	struct FChaosClothWeightedValue               AnimDriveDamping;                                  // 0x0118(0x0008)
	struct FVector                                LinearVelocityScale;                               // 0x0120(0x0018)
	float                                         AngularVelocityScale;                              // 0x0138(0x0004)
	float                                         FictitiousAngularScale;                            // 0x013C(0x0004)
	bool                                          bUseTetrahedralConstraints;                        // 0x0140(0x0001)
	bool                                          bUseThinShellVolumeConstraints;                    // 0x0141(0x0001)
	bool                                          bUseContinuousCollisionDetection;                  // 0x0142(0x0001)
	uint8                                         Pad_143[0x5];                                      // 0x0143(0x0005)

public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ChaosClothConfig")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ChaosClothConfig")
	}
	static class UChaosClothConfig* GetDefaultObj()
	{
		return GetDefaultObjImpl<UChaosClothConfig>();
	}
};

// Class ChaosCloth.ChaosClothSharedSimConfig
// 0x0010 (0x0038 - 0x0028)
class UChaosClothSharedSimConfig final : public UClothSharedConfigCommon
{
public:
	int32                                         IterationCount;                                    // 0x0028(0x0004)
	int32                                         MaxIterationCount;                                 // 0x002C(0x0004)
	int32                                         SubdivisionCount;                                  // 0x0030(0x0004)
	bool                                          bUseLocalSpaceSimulation;                          // 0x0034(0x0001)
	bool                                          bUseXPBDConstraints;                               // 0x0035(0x0001)
	uint8                                         Pad_36[0x2];                                       // 0x0036(0x0002)

public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ChaosClothSharedSimConfig")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ChaosClothSharedSimConfig")
	}
	static class UChaosClothSharedSimConfig* GetDefaultObj()
	{
		return GetDefaultObjImpl<UChaosClothSharedSimConfig>();
	}
};

// Class ChaosCloth.ChaosClothingSimulationFactory
// 0x0000 (0x0028 - 0x0028)
class UChaosClothingSimulationFactory final : public UClothingSimulationFactory
{
public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ChaosClothingSimulationFactory")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ChaosClothingSimulationFactory")
	}
	static class UChaosClothingSimulationFactory* GetDefaultObj()
	{
		return GetDefaultObjImpl<UChaosClothingSimulationFactory>();
	}
};

// Class ChaosCloth.ChaosClothingInteractor
// 0x0020 (0x0050 - 0x0030)
class UChaosClothingInteractor final : public UClothingInteractor
{
public:
	uint8                                         Pad_30[0x20];                                      // 0x0030(0x0020)

public:
	void ResetAndTeleport(bool bReset, bool bTeleport);
	void SetAerodynamics(float DragCoefficient, float LiftCoefficient, const struct FVector& WindVelocity);
	void SetAnimDrive(const struct FVector2D& AnimDriveStiffness, const struct FVector2D& AnimDriveDamping);
	void SetAnimDriveLinear(float AnimDriveStiffness);
	void SetBackstop(bool bEnabled);
	void SetCollision(float CollisionThickness, float FrictionCoefficient, bool bUseCCD, float SelfCollisionThickness);
	void SetDamping(float DampingCoefficient, float LocalDampingCoefficient);
	void SetGravity(float GravityScale, bool bIsGravityOverridden, const struct FVector& GravityOverride);
	void SetLongRangeAttachment(const struct FVector2D& TetherStiffness, const struct FVector2D& TetherScale);
	void SetLongRangeAttachmentLinear(float TetherStiffness, float TetherScale);
	void SetMaterial(const struct FVector2D& EdgeStiffness, const struct FVector2D& BendingStiffness, const struct FVector2D& AreaStiffness);
	void SetMaterialBuckling(const struct FVector2D& BucklingRatio, const struct FVector2D& BucklingStiffness);
	void SetMaterialLinear(float EdgeStiffness, float BendingStiffness, float AreaStiffness);
	void SetPressure(const struct FVector2D& Pressure);
	void SetVelocityScale(const struct FVector& LinearVelocityScale, float AngularVelocityScale, float FictitiousAngularScale);
	void SetWind(const struct FVector2D& Drag, const struct FVector2D& Lift, float AirDensity, const struct FVector& WindVelocity, const struct FVector2D& OuterDrag, const struct FVector2D& OuterLift);

public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ChaosClothingInteractor")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ChaosClothingInteractor")
	}
	static class UChaosClothingInteractor* GetDefaultObj()
	{
		return GetDefaultObjImpl<UChaosClothingInteractor>();
	}
};

// Class ChaosCloth.ChaosClothingSimulationInteractor
// 0x0020 (0x00B0 - 0x0090)
class UChaosClothingSimulationInteractor final : public UClothingSimulationInteractor
{
public:
	uint8                                         Pad_90[0x20];                                      // 0x0090(0x0020)

public:
	static class UClass* StaticClass()
	{
		STATIC_CLASS_IMPL("ChaosClothingSimulationInteractor")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"ChaosClothingSimulationInteractor")
	}
	static class UChaosClothingSimulationInteractor* GetDefaultObj()
	{
		return GetDefaultObjImpl<UChaosClothingSimulationInteractor>();
	}
};

}

